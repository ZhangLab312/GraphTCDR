...
def read_drug_smiles(filename, drug_list):  # load drugs' SMILES
    drug_number = drug_list.shape[0]
    smiles = pd.read_csv(filename, index_col=0)
    drug_smiles = [list() for i in range(drug_number)]
    for i in range(drug_number):
        cid = drug_list.iloc[i, 1]
        drug_smiles[i] = smiles.loc[[cid]]['CanonicalSMILES'].values[0]  # use the index in dictionary
    return drug_smiles
...
def read_response_data_and_process():
    # load features
    drug_list = pd.read_csv('../data/drug/drugNameInfo1353.csv')
    finger = torch.load('../data/drug/fingerPrintFeature1353.pt')
    smile = read_drug_smiles('data/drug/drug_smiles.csv', drug_list)
    smile_graph = get_all_graph(smile)
    cell_line_dict = pd.read_csv('../data/cell_line/cellLineName402.csv', index_col=0)
    miRNA = torch.load('../data/cell_line/miRNAFeature402.pt')
    copynumber = torch.load('../data/cell_line/512dim_CNV_AE.pt')
    copynumber = copynumber.to('cpu')

    scale = ''   # 全部数据则设置为空： ''

    data = torch.tensor(pd.read_csv('../data/unknownPairs/unknownPairsIndexData.csv'.format(scale)).to_numpy())
    ...
    for index, item in enumerate(data):
        ...
        drug_smile.append(smile[drug.int()])
        drug_finger.append(finger[drug.int()])
        cell_miRNA.append(miRNA[cell_line.int()])
        cell_copy.append(copynumber[cell_line.int()])
        label.append(ic50)
    ...
    for i in range(5):
        ...
        TestbedDataset(root='data', dataset='train_set_sample{num}{scale}'.format(num=i, scale=scale), xdf=drugfinger_train,
                       xds=drugsmile_train,
                       xcm=cellmiRNA_train, xcc=cellcopy_train,
                       y=label_train, smile_graph=smile_graph)
        TestbedDataset(root='data', dataset='val_set_sample{num}{scale}'.format(num=i, scale=scale), xdf=drugfinger_val,
                       xds=drugsmile_val,
                       xcm=cellmiRNA_val, xcc=cellcopy_val,
                       y=label_val, smile_graph=smile_graph)
        TestbedDataset(root='data', dataset='test_set_sample{num}{scale}'.format(num=i, scale=scale), xdf=drugfinger_test,
                       xds=drugsmile_test,
...
def read_response_data_and_process_Unknown_pairs():
    # load features
    drug_list = pd.read_csv('../data/drug/drugNameInfo1353.csv')
    finger = torch.load('../data/drug/fingerPrintFeature1353.pt')
    smile = read_drug_smiles('data/drug/drug_smiles.csv', drug_list)
    ...
    # data = torch.tensor(pd.read_csv('../data/unknownPairs/unknownPairsIndexData.csv').to_numpy())
    data = torch.tensor(pd.read_csv('../data/drug_response_data_number_log_4_5_1_16.csv').to_numpy())
    data = data[torch.randperm(data.size(0))]
    ...
    total_size = drug_smile.shape[0]
    # features of drug fingers
    drugfinger_test = drug_finger
    # features of drug smiles
    drugsmile_test = drug_smile
    # features of cell miRNA
    cellmiRNA_test = cell_miRNA
    # features of cell copynumber
    cellcopy_test = cell_copy
    # label
    label_test = label
    TestbedDataset(root='data', dataset='test_set_1_16', xdf=drugfinger_test,
                   xds=drugsmile_test,
                   xcm=cellmiRNA_test, xcc=cellcopy_test,
                   y=label_test, smile_graph=smile_graph)
...
def process_blind_cell(filename):
    # load features
    drug_dict, drug_properties = read_drug_list_and_properties('../data/drug/physicochemicalFeature1353.pt')
    finger = read_drug_finger('../data/drug/fingerPrintFeature1353.pt', drug_dict)
...
if __name__ == "__main__":
    ...
    read_response_data_and_process_Unknown_pairs()
