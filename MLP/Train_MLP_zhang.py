...
SaveResultAddress = '../SaveResult/compared/MLPResult.csv'
saveResultAndModel = True


def loadData(N, i):
    mirna_feature = torch.load('../data/cell_line/miRNAFeature402.pt').to(device)
    gene_expression_feature = torch.load('../data/cell_line/512dim_geneExpression_AE.pt', map_location=device)
    CNV_feature = torch.load('../data/cell_line/512dim_CNV_AE.pt', map_location=device)
    meth = torch.load('../data/cell_line/512dim_Methylation_AE.pt', map_location=device)

    phy = torch.load('../data/drug/physicochemicalFeature1353.pt')
    phy = torch.tensor(phy, dtype=torch.float).to(device)
    fingerprint_feature = torch.load('../data/drug/fingerPrintFeature1353.pt').to(device)
